...
import { calcularFlujoFrances } from './francesMetod.js';
 ...
 */
export function calcularTCEA(params) {
  const {
    valorNominal,
    tasaAnual,
    frecuenciaPago,
 ...
 */
function calcularVPNyDerivada(flujoEmisor, tasaPeriodo) {
  let vpn = 0;
  let derivada = 0;
  ...
  for (const flujo of flujoEmisor) {
    ...
    derivada -= (flujo.periodo * flujo.flujoNeto) / (factor * (1 + tasaPeriodo));
  }
