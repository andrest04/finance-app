  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 max-w-6xl mx-auto">
      ...
      <section className="border border-blue-300 rounded-md p-4 space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <Input name="valorNominal" type="number" value={form.valorNominal} onChange={handleChange} />
          </div>
          <div>
            <Label>Moneda</Label>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div>
            ...
            <Select onValueChange={(val) => handleSelect("tipoTasa", val)}>
              <SelectTrigger><SelectValue placeholder="Nominal o Efectiva" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            ...
            <Input name="tasaAnual" type="number" value={form.tasaAnual} onChange={handleChange} />
          </div>
          <div>
            <Label>Frecuencia de Pago (f)</Label>
            <Select onValueChange={(val) => handleSelect("frecuenciaPago", val)}>
          ...
          {form.tipoTasa === "Nominal" && (
            <div className="md:col-span-1">
              ...
              <Select onValueChange={(val) => handleSelect("frecuenciaCapitalizacion", val)}>
                <SelectTrigger><SelectValue placeholder="Capitalización" /></SelectTrigger>
                <SelectContent>
      ...
      <section className="border border-blue-300 rounded-md p-4 space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div>
            ...
            <Input name="plazo" type="number" value={form.plazo} onChange={handleChange} />
          </div>
          <div>
            <Label>Tipo de Gracia</Label>
            <Select onValueChange={(val) => handleSelect("tipoGracia", val)}>
              <SelectTrigger><SelectValue placeholder="Tipo de gracia" /></SelectTrigger>
              <SelectContent>
          ...
          {(form.tipoGracia === "Parcial" || form.tipoGracia === "Total") && (
            <div>
              ...
              <Input name="nGracia" type="number" value={form.nGracia} onChange={handleChange} />
            </div>
          )}
          <div>
            <Label>Fecha de Emisión</Label>
            <Input name="fechaEmision" type="date" value={form.fechaEmision} onChange={handleChange} />
          </div>
        </div>
      </section>
      ...
      <section className="border border-blue-300 rounded-md p-4 space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div>
            ...
            <Input name="comisionEmisor" type="number" value={form.comisionEmisor} onChange={handleChange} />
          </div>
          <div>
            <Label>Comisión Bonista (%)</Label>
            <Input name="comisionBonista" type="number" value={form.comisionBonista} onChange={handleChange} />
          </div>
          <div>
            <Label>Tasa Mercado (TREA)</Label>
            <Input name="tasaMercado" type="number" value={form.tasaMercado} onChange={handleChange} />
          </div>
        </div>
      </section>
      ...
      <div className="flex justify-end gap-4">
        <Button variant="outline" type="button" onClick={() => console.log("Cancelado")}>
          Cancelar
        </Button>
